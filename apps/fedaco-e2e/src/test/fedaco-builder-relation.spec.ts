...
import {
  FedacoBuilderTestModelCloseRelatedStub,
  FedacoBuilderTestModelParentStub
...
describe('fedaco builder relation', () => {
  ...
  it('test has with constraints with or where and having inSubquery', () => {
    ...
    builder
      .whereHas('address', (q: FedacoBuilder) => {
  ...
  it('test has nested with constraints', () => {
    ...
    const result = model
      .NewQuery()
      .whereHas('foo.bar', (q: FedacoBuilder) => {
        q.where('baz', 'bam');
      })
      .toSql();
  ...
  it('test self has nested', () => {
    ...
    const nestedSql = model
      .NewQuery()
      .whereHas('parentFoo', (q: FedacoBuilder) => {
        q.has('childFoo');
      })
      .toSql();
    const dotSql = model.NewQuery().has('parentFoo.childFoo').toSql();
    const alias = 'self_alias_hash';
    const aliasRegex = /\b(fedaco_reserved_\d)(\b|$)/gi;
  ...
  it('test or where doesnt have', () => {
    ...
    const builder = model
      .NewQuery()
      .where('bar', 'baz')
      .orWhereDoesntHave('foo', (q: FedacoBuilder) => {
        q.where('qux', 'quux');
      });
