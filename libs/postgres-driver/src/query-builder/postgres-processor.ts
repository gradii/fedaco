...
import { Processor, type QueryBuilder } from '@gradii/fedaco';

export class PostgresProcessor extends Processor {
  /* Process an "insert get ID" query. */
  public async processInsertGetId(query: QueryBuilder, sql: string, values: any[], sequence?: string) {
    ...
    return results.map((result) => {
      return {
        ...
        category:
          ...
          )[result.category.toLowerCase()] || null,
      };
    });
  }
    ...
    return results.map((result) => {
      return {
        ...
        on_update:
          ...
          )[result.on_update.toLowerCase()] || null,
        on_delete:
          (
            {
              a: 'no action',
          ...
          )[result.on_delete.toLowerCase()] || null,
      };
    });
  }
}
