...
describe('database query builder test', () => {
  ...
  it('test where date sqlite', () => {
    ...
    expect(builder.toSql())
      .toBe('SELECT * FROM "users" WHERE strftime(\'%Y-%m-%d\', "created_at") = cast(NOW() AS text)');
  });
  ...
  it('test basic where column', () => {
    ...
    expect(builder.toSql())
      .toBe('SELECT * FROM `users` WHERE `first_name` = `last_name` OR `first_name` = `middle_name`');
    expect(builder.getBindings()).toStrictEqual([]);
    builder = getBuilder();
  ...
  it('test union order bys', () => {
    ...
    expect(builder.toSql())
      .toBe('(SELECT * FROM `users` WHERE `id` = ?) UNION (SELECT * FROM `users` WHERE `id` = ?) ORDER BY `id` DESC');
    expect(builder.getBindings()).toStrictEqual([1, 2]);
  });
  ...
  it('test sub select where ins', () => {
    ...
    expect(builder.toSql())
      .toBe('SELECT * FROM `users` WHERE `id` IN (SELECT `id` FROM `users` WHERE `age` > ? LIMIT 3)');
    ...
    expect(builder.toSql())
      .toBe('SELECT * FROM `users` WHERE `id` NOT IN (SELECT `id` FROM `users` WHERE `age` > ? LIMIT 3)');
    expect(builder.getBindings()).toStrictEqual([25]);
  });
  ...
  it('test raw havings', () => {
    ...
    expect(builder.toSql())
      .toBe('SELECT * FROM `users` HAVING `last_login_date` BETWEEN ? AND ? OR user_foo < user_bar');
  });
  ...
  it('test get count for pagination with bindings', () => {
  ...
  it('test get count for pagination with column aliases', () => {
    let spySelector, spyProcessSelector;
    builder = getBuilder();
    ...
    expect(spySelector)
      .toBeCalledWith('SELECT count(`body`, `teaser`, `posts`.`created`) AS aggregate FROM `posts`', [], true);

    expect(count).toBe(1);
  });

  it('test get count for pagination with union', () => {
    let spySelector, spyProcessSelector;
    builder = getBuilder();
  ...
  it('test full sub selects', () => {
    ...
    expect(builder.toSql())
      .toBe('SELECT * FROM `users` WHERE `email` = ? OR `id` = (SELECT max(id) FROM `users` WHERE `email` = ?)');
    expect(builder.getBindings()).toStrictEqual(['foo', 'bar']);
  });
  ...
  it('test where exists', () => {
    ...
    expect(builder.toSql())
      .toBe('SELECT * FROM `orders` WHERE EXISTS (SELECT * FROM `products` WHERE `products`.`id` = `orders`.`id`)');
    builder = getBuilder();
    builder.select('*').from('orders').whereNotExists(q => {
      q.select('*').from('products').where('products.id', '=', raw('`orders`.`id`'));
    });
    expect(builder.toSql())
      .toBe('SELECT * FROM `orders` WHERE NOT EXISTS (SELECT * FROM `products` WHERE `products`.`id` = `orders`.`id`)');
    builder = getBuilder();
  ...
  it('test basic joins', () => {
    ...
    expect(builder.toSql())
      .toBe('SELECT * FROM `users` LEFT JOIN `photos` ON `users`.`id` = ? INNER JOIN `photos` ON `users`.`id` = ?');
  ...
  it('test cross joins', () => {
    ...
    expect(builder.toSql())
      .toBe('SELECT * FROM `tableB` CROSS JOIN `tableA` ON `tableA`.`column1` = `tableB`.`column2`');
    builder = getBuilder();
  ...
  it('test complex join', () => {
    ...
    expect(builder.toSql())
      .toBe('SELECT * FROM `users` INNER JOIN `contacts` ON `users`.`id` = ? OR `users`.`name` = ?');
    expect(builder.getBindings()).toStrictEqual(['foo', 'bar']);
    expect(builder.toSql())
      .toBe('SELECT * FROM `users` INNER JOIN `contacts` ON `users`.`id` = ? OR `users`.`name` = ?');
    expect(builder.getBindings()).toStrictEqual(['foo', 'bar']);
  ...
  it('test joins with nested join with advanced subquery condition', () => {
    ...
    builder.select('users.id', 'contacts.id', 'contact_types.id').from('users').leftJoin('contacts', j => {
      j.on('users.id', 'contacts.id')
        .join('contact_types', 'contacts.contact_type_id', '=', 'contact_types.id')
        .whereExists(q => {
          q.select('*').from('countrys').whereColumn('contacts.country', '=', 'countrys.country').join('planets', q => {
  ...
  it('test left join sub', () => {
    ...
    expect(builder.toSql())
      .toBe('SELECT * FROM `users` LEFT JOIN (SELECT * FROM `contacts`) AS `sub` ON `users`.`id` = `sub`.`id`');
    builder = getBuilder();
    expect(() => {
      // @ts-ignore
  ...
  it('test right join sub', () => {
    ...
    expect(builder.toSql())
      .toBe('SELECT * FROM `users` RIGHT JOIN (SELECT * FROM `contacts`) AS `sub` ON `users`.`id` = `sub`.`id`');
    builder = getBuilder();
    expect(() => {
      // @ts-ignore
  ...
  it('test first method returns first result', () => {
    let spySelect, spyProcessSelect;
    builder = getBuilder();
...
*/
  ...
  it('test aggregate reset followed by get with columns', () => {
  ...
  it('test update method without joins on postgres', () => {
    ...
    expect(spyUpdate)
      .toBeCalledWith('UPDATE "users"."users" SET "email" = ?, "name" = ? WHERE "id" = ?', ['foo', 'bar', 1]);
    expect(result).toBe(1);
  });
  ...
  it('test update or insert method works with empty update values without from', () => {
    ...
    }).toThrowError('must call from before insert');
  ...
  it('test postgres insert get id', () => {
    ...
    expect(spyInsertGetId).toBeCalledWith('INSERT INTO "users" ("email") VALUES (?) returning "id"', ['foo'], 'id')
    expect(result).toBe(1)
  });
  ...
  it('test sql server insert get id', () => {
    ...
    expect(spyInsertGetId).toBeCalledWith('set nocount on;INSERT INTO [users] ([email]) VALUES (?);select scope_identity() as [id]',
      ['foo'], 'id');
    expect(result).toBe(1)
  });
  ...
  //   builder.getConnection().shouldReceive('select').once()._with(m.any(), m.any(), false);
  ...
  //   let expectedSql      = 'SELECT * FROM "users" inner join "othertable" on "bar" = ? where "registered" = ? group by "city" having "population" > ? ORDER BY match ("foo") against(?)';
  //   let expectedBindings = ['foo', 1, 3, 'bar'];
  //   let builder          = getBuilder();
  //   builder.select('*')
  //     .from('users')
  ...
  //
  // it('test sub select', () => {
  //   let expectedSql      = 'select "foo", "bar", (select "baz" from "two" where "subkey" = ?) as "sub" from "one" where "key" = ?';
  //   let expectedBindings = ['subval', 'val'];
  ...
  //   builder = getPostgresBuilder();
  //   builder.from('one').select(['foo', 'bar']).where('key', '=', 'val');
  //   let subBuilder = getPostgresBuilder();
  ...
  //     query.from('two').select('baz').where('subkey', '=', 'subval');
  //   }, 'sub');
  //   this.assertEquals('select (select "baz" from "two" where "subkey" = ?) as "sub" from "one"',
  //     builder.toSql());
  ...
  // });
  //
  // it('test table valued function as table in sql server', () => {
  //   builder = getSqlServerBuilder();
  //   builder.select('*').from('users()');
  //   expect(builder.toSql()).toBe('SELECT * FROM [users]()');
  //   builder = getSqlServerBuilder();
  ...
  //   expect(builder.toSql())
  //     .toBe('SELECT * FROM `users` where `id` = ? or json_contains(`options`, \'["en"]\', \'$."languages"\')');
  //   expect(builder.getBindings()).toStrictEqual([1]);
  // });
  //
  ...
  //     .orWhereJsonContains('options->languages', raw('\'["en\\"]\''));
  //   expect(builder.toSql())
  //     .toBe('SELECT * FROM "users" WHERE "id" = ? or ("options"->\'languages\')::jsonb @> \'["en"]\'');
  //   expect(builder.getBindings()).toStrictEqual([1]);
  // });
  //
  ...
  //   builder.select('*').from('users').whereJsonContains('users.options->languages', 'en');
  //   expect(builder.toSql())
  //     .toBe('SELECT * FROM [users] where ? in (select [value] from openjson([users].[options], \'$."languages"\'))');
  //   expect(builder.getBindings()).toStrictEqual(['en']);
  //   builder = getSqlServerBuilder();
  ...
  //   expect(builder.toSql())
  //     .toBe('SELECT * FROM `users` where `id` = ? or not json_contains(`options`, \'["en"]\', \'$."languages"\')');
  //   expect(builder.getBindings()).toStrictEqual([1]);
  // });
  //
  ...
  //     .orWhereJsonDoesntContain('options->languages', raw('\'["en\\"]\''));
  //   expect(builder.toSql())
  //     .toBe('SELECT * FROM "users" WHERE "id" = ? or not ("options"->\'languages\')::jsonb @> \'["en"]\'');
  //   expect(builder.getBindings()).toStrictEqual([1]);
  // });
  //
  ...
  //   builder.select('*').from('users').whereJsonDoesntContain('options->languages', 'en');
  //   expect(builder.toSql())
  //     .toBe('SELECT * FROM [users] where not ? in (select [value] from openjson([options], \'$."languages"\'))');
  //   expect(builder.getBindings()).toStrictEqual(['en']);
  //   builder = getSqlServerBuilder();
  //   builder.select('*')
  //     .from('users')
  ...
  //   expect(builder.toSql())
  //     .toBe('SELECT * FROM `users` where `id` = ? or json_length(`options`, \'$."languages"\') = 0');
  //   expect(builder.getBindings()).toStrictEqual([1]);
  //   builder = getMySqlBuilder();
  //   builder.select('*').from('users').where('id', '=', 1).orWhereJsonLength('options->languages', '>', raw('0'));
  //   expect(builder.toSql())
  ...
  //   builder = getPostgresBuilder();
  //   builder.select('*').from('users').whereJsonLength('options', 0);
  //   expect(builder.toSql()).toBe('SELECT * FROM "users" where json_array_length(("options")::json) = ?');
  //   expect(builder.getBindings()).toStrictEqual([0]);
  //   builder = getPostgresBuilder();
  //   builder.select('*').from('users').whereJsonLength('users.options->languages', '>', 0);
  //   expect(builder.toSql())
  //     .toBe('SELECT * FROM "users" where json_array_length(("users"."options"->\'languages\')::json) > ?');
  //   expect(builder.getBindings()).toStrictEqual([0]);
  //   builder = getPostgresBuilder();
  //   builder.select('*').from('users').where('id', '=', 1).orWhereJsonLength('options->languages', raw('0'));
  //   expect(builder.toSql())
  //     .toBe('SELECT * FROM "users" WHERE "id" = ? or json_array_length(("options"->\'languages\')::json) = 0');
  //   expect(builder.getBindings()).toStrictEqual([1]);
  //   builder = getPostgresBuilder();
  //   builder.select('*').from('users').where('id', '=', 1).orWhereJsonLength('options->languages', '>', raw('0'));
  //   expect(builder.toSql())
  ...
  //   builder.select('*').from('users').whereJsonLength('users.options->languages', '>', 0);
  //   expect(builder.toSql())
  //     .toBe('SELECT * FROM "users" where json_array_length("users"."options", \'$."languages"\') > ?');
  //   expect(builder.getBindings()).toStrictEqual([0]);
  //   builder = getSQLiteBuilder();
  //   builder.select('*').from('users').where('id', '=', 1).orWhereJsonLength('options->languages', raw('0'));
  //   expect(builder.toSql())
  //     .toBe('SELECT * FROM "users" WHERE "id" = ? or json_array_length("options", \'$."languages"\') = 0');
  //   expect(builder.getBindings()).toStrictEqual([1]);
  //   builder = getSQLiteBuilder();
  //   builder.select('*').from('users').where('id', '=', 1).orWhereJsonLength('options->languages', '>', raw('0'));
  //   expect(builder.toSql())
  ...
  //   builder.select('*').from('users').whereJsonLength('users.options->languages', '>', 0);
  //   expect(builder.toSql())
  //     .toBe('SELECT * FROM [users] where (select count(*) from openjson([users].[options], \'$."languages"\')) > ?');
  //   expect(builder.getBindings()).toStrictEqual([0]);
  //   builder = getSqlServerBuilder();
  //   builder.select('*').from('users').where('id', '=', 1).orWhereJsonLength('options->languages', raw('0'));
  //   expect(builder.toSql())
  ...
  //   }, 'sessions');
  //   expect(builder.toSql())
  //     .toBe('SELECT * FROM (select max(last_seen_at) as last_seen_at from "user_sessions") as "sessions"');
  //   this.expectException(InvalidArgumentException);
  //   builder = getBuilder();
  ...
  // it('test from raw', () => {
  //   builder = getBuilder();
  //   builder.fromRaw(raw('(select max(last_seen_at) as last_seen_at from "user_sessions") as "sessions"'));
  //   expect(builder.toSql())
  //     .toBe('SELECT * FROM (select max(last_seen_at) as last_seen_at from "user_sessions") as "sessions"');
  // });
  //
  ...
  //   builder = getBuilder();
  //   builder.fromRaw(raw('(select max(last_seen_at) as last_seen_at from "sessions") as "last_seen_at"'))
