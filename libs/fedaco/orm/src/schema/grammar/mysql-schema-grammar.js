...
export class MysqlSchemaGrammar extends SchemaGrammar {
  ...
  compileCreateDatabase(name, connection) {
    return `create database ${this.wrapValue(
      name
    )} default character set ${this.wrapValue(
      connection.getConfig('charset')
    )} default collate ${this.wrapValue(connection.getConfig('collation'))}`
  }
