...
export class PostgresSchemaGrammar extends SchemaGrammar {
  ...
  compileCreate(blueprint, command) {
    return ([
      ...[
        `${blueprint._temporary ? 'create temporary' : 'create'} table ${this.wrapTable(blueprint)} (${this.getColumns(blueprint).join(', ')})`
  ...
  compileAdd(blueprint, command) {
    return ([
      ...[
        `alter table ${this.wrapTable(blueprint)} ${this.prefixArray('add column', this.getColumns(blueprint)).join(', ')}`
      ], ...this.compileAutoIncrementStartingValues(blueprint)
    ]).filter(it => !!it);
  }
  ...
  compileUnique(blueprint, command) {
    return 'alter table ' + `${this.wrapTable(blueprint)} add constraint ${this.wrap(command.index)} unique (${this.columnize(command.columns)})`;
  }

  compileIndex(blueprint, command) {

    return 'create index ' + `${this.wrap(command.index)} on ${this.wrapTable(blueprint)}${command.algorithm ? ' using ' + command.algorithm : ''} (${this.columnize(command.columns)})`;
  }
  ...
  compileComment(blueprint, command) {
    return `comment on column ${this.wrapTable(blueprint)}.${this.wrap(command.get('column').name)} is ${`'${command.get('column').comment.replace(/'/g, `''`)}'`}`;
  }
  ...
  typeTime(column) {
    return `time${isBlank(column.precision) ?
      '' : `(${column.precision})`} without time zone`;
  }
  ...
  typeTimestamp(column) {
    const columnType = `timestamp${isBlank(column.precision) ?
      '' : `(${column.precision})`} without time zone`;
  ...
  formatPostGisType(type, column) {
    ...
    if (column.isGeometry === null) {
      return `geography(${type}, ${(_a = column.projection) !== null && _a !== void 0 ? _a : '4326'})`;
    }
  ...
  modifyIncrement(blueprint, column) {
    if ((this.serials.includes(column.type) ||
      column.generatedAs !== null) && column.autoIncrement) {
      return ' primary key';
    }
    return '';
  }
