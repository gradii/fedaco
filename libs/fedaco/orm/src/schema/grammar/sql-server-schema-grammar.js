...
export class SqlServerSchemaGrammar extends SchemaGrammar {
  constructor() {
    ...
    this.modifiers = ['Increment', 'Collate', 'Nullable', 'Default', 'Persisted'];
  ...
  compileUnique(blueprint, command) {
    ...
    return `create
    unique index ` + `${this.wrap(command.index)} on ${this.wrapTable(blueprint)} (${this.columnize(command.columns)})`;
  }
  ...
  compileDropIfExists(blueprint, command) {
    return `if exists (select * from sys.sysobjects where id = object_id(${`'${this.getTablePrefix()}${blueprint.getTable().replace(`'`, `''`)}'`}, 'U')) drop table ${this.wrapTable(blueprint)}`;
  }
