...
export class SqliteSchemaBuilder extends SchemaBuilder {
  ...
  _getPortableTableColumnDefinition(tableColumn) {
    ...
    let length = (_a = tableColumn['length']) !== null && _a !== void 0 ? _a : null;
    let unsigned = false;
    ...
    switch (dbType) {
      ...
      case 'numeric':
        if (tableColumn['length'] !== undefined) {
          ...
          [precision, scale] = tableColumn['length'].split(',').map((it) => it.trim());
        }
        length = null;
        break;
    }
...
_SqliteSchemaBuilder_instances = new WeakSet(), _SqliteSchemaBuilder_getCreateTableSQL = function _SqliteSchemaBuilder_getCreateTableSQL(table) {
  return __awaiter(this, void 0, void 0, function* () {
    return (yield this.connection
      .select(`SELECT sql
               FROM (SELECT * FROM sqlite_master UNION ALL SELECT * FROM sqlite_temp_master)
               WHERE type = 'table' AND name = ?`, [table])) || null;
  });
};
