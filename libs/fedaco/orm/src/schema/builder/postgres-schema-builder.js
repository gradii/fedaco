...
export class PostgresSchemaBuilder extends SchemaBuilder {
  ...
  hasTable(table) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const result = yield this.connection.select(this.grammar.compileTableExists(), [database, schema, table]);
  ...
  dropAllTables() {
    ...
    return __awaiter(this, void 0, void 0, function* () {
      const tables = [];
      const excludedTables = (_a = this.connection.getConfig('dont_drop')) !== null && _a !== void 0 ? _a : ['spatial_ref_sys'];
  ...
  getAllTables() {
    return this.connection.select(this.grammar.compileGetAllTables(this.parseSearchPath(this.connection.getConfig('search_path'))));
  }

  getAllViews() {
    return this.connection.select(this.grammar.compileGetAllViews(this.parseSearchPath(this.connection.getConfig('search_path'))));
  }
  ...
  getColumnListing(table) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      [database, schema, table] = this.parseSchemaAndTable(table);
      table = this.connection.getTablePrefix() + table;
      const results = yield this.connection.select(this.grammar.compileColumnListing(), [database, schema, table]);
  ...
  parseSchemaAndTable(reference) {
    ...
    let schema = searchPath[0] === '$user' ? this.connection.getConfig('username') : searchPath[0];
    if (parts.length === 2) {
      schema = parts[0];
      parts.shift();
    }
