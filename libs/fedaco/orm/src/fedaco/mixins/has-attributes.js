    ...
    (_a = class _Self extends base {
      ...
      addCastAttributesToArray(attributes, mutatedAttributes) {
        for (const [key, value] of Object.entries(this.getCasts())) {
          ...
          if (
            attributes[key] &&
            attributes[key] instanceof Date &&
      ...
      getAttribute(key) {
        ...
        if (this._attributes.hasOwnProperty(key) || this.isClassCastable(key)) {
      ...
      _getAttributeFromArray(key) {
        var _a
        return (_a = this.getAttributes()[key]) !== null && _a !== void 0
          ? _a
          : null
      }
      ...
      isImmutableCustomDateTimeCast(cast) {
        return (
          cast.startsWith('immutable_date:') ||
      ...
      _columnInfo(key) {
        ...
        if (meta[key] && isArray(meta[key])) {
          return findLast((it) => {
            return (
              FedacoColumn.isTypeOf(it) || FedacoRelationColumn.isTypeOf(it)
            )
          }, meta[key])
        }
        return undefined
      ...
      castAttributeAsEncryptedString(key, value) {
        var _a
        return (
          (_a = this.constructor.encrypter) !== null && _a !== void 0
            ? _a
            : Crypt.getCryptor()
        ).encrypt(value, false)
      }
      ...
      asDateTime(value) {
        ...
        try {
          date = parse(
            value,
            this.getDateFormat() || 'yyyy-MM-dd HH:mm:ss',
            new Date(value)
          )
        } catch (e) {}
      ...
      getOriginalWithoutRewindingModel(key = null, _default = null) {
        if (key) {
          return this.transformModelValue(
            key,
            get(this._original, key, _default)
          )
        }
        const results = {}
      ...
      originalIsEquivalent(key) {
        ...
        return (
          isNumber(attribute) && isNumber(original) && attribute === original
        )
      }
      ...
      append(attributes) {
        this._appends = uniq([
          ...this._appends,
          ...(isString(attributes) ? arguments : attributes),
        ])
        return this
      }
