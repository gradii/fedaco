...
export function mixinHasEvents(base) {
  ...
  return class _Self extends base {
    ...
    removeObservableEvents(observables) {
      this._observables = difference(this._observables, isArray(observables) ? observables : [...arguments]);
    }
    ...
    _registerObserver(clazz) {
      ...
      for (const event of this.getObservableEvents()) {
        if (event in clazz) {
          this.constructor._registerModelEvent(event, (...args) => clazz.prototype[event].apply());
        }
      }
    }
    ...
    _fireModelEvent(event, halt = true) {
      ...
      return !isAnyEmpty(result) ?
        result :
        this.constructor.dispatcher[method](`eloquent.${event}: ${this.constructor.name}`, this);
    }
    ...
    _fireCustomModelEvent(event, method) {
      ...
      const result = this.constructor.dispatcher[method](new this._dispatchesEvents[event](this));
      if (!isBlank(result)) {
        return result;
      }
    }
