...
export class MorphTo extends BelongsTo {
  ...
  buildDictionary(models) {
    for (const model of models) {
      if (model._getAttributeFromArray(this._morphType)) {
        const morphTypeKey = this._getDictionaryKey(model._getAttributeFromArray(this._morphType));
        const foreignKeyKey = this._getDictionaryKey(model._getAttributeFromArray(this._foreignKey));
  ...
  getResultsByType(clazz) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const ownerKey = (_a = this._ownerKey) !== null && _a !== void 0 ? _a : instance.getKeyName();
      const query = this.replayMacros(instance.newQuery())
        .mergeConstraintsFrom(this.getQuery())
        .with(Object.assign(Object.assign({}, this.getQuery().getEagerLoads()), (_b = this._morphableEagerLoads.get(instance.constructor)) !== null && _b !== void 0 ? _b : {})).withCount((_c = this._morphableEagerLoadCounts.get(instance.constructor)) !== null && _c !== void 0 ? _c : []);
      const callback = (_d = this._morphableConstraints.get(instance.constructor)) !== null && _d !== void 0 ? _d : null;
      if (callback) {
        callback(query);
      }
      const whereIn = this.whereInMethod(instance, ownerKey);
      return query[whereIn](instance.getTable() + '.' + ownerKey, this.gatherKeysByType(clazz, instance.getKeyType())).get();
    });
  }
  ...
  associate(model) {
    ...
    this._parent.setAttribute(this._morphType, model instanceof Model ? model.getMorphClass() : null);
