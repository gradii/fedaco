...
export class BelongsToMany extends mixinInteractsWithDictionary(mixinInteractsWithPivotTable(Relation)) {
  ...
  _buildDictionary(results) {
    ...
    for (const result of results) {
      const value = this._getDictionaryKey(result.getRelation(this._accessor).getAttributeValue(this._foreignPivotKey));
  ...
  get(columns = ['*']) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      let models = yield builder.addSelect(this._shouldSelect(columns)).getModels();
  ...
  chunkById(count, column, alias, signal) {
    this._prepareQueryBuilder();
    column = column !== null && column !== void 0 ? column : this.getRelated().qualifyColumn(this.getRelatedKeyName());
    alias = alias !== null && alias !== void 0 ? alias : this.getRelatedKeyName();
    return this._query.chunkById(count, column, alias).pipe(tap(({ results }) => {
      this._hydratePivotRelation(results);
    }));
  }
  ...
  touch() {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      if (ids.length > 0) {
        yield this.getRelated().newQueryWithoutRelationships().whereIn(key, ids).update(columns);
      }
    });
  }
  ...
  saveMany(models, pivotAttributes = []) {
    ...
    for (const [key, model] of Object.entries(models)) {
      this.save(model, (_a = pivotAttributes[key]) !== null && _a !== void 0 ? _a : [], false);
    }
  ...
  createMany(records, joinings = []) {
    ...
    for (const [key, record] of Object.entries(records)) {
      instances.push(this.create(record, (_a = joinings[key]) !== null && _a !== void 0 ? _a : [], false));
    }
    this.touchIfTouching();
    return instances;
  }
