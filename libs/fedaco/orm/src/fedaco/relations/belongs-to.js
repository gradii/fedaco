...
export class BelongsTo extends mixinComparesRelatedModels(mixinInteractsWithDictionary(mixinSupportsDefaultModels(Relation))) {
  ...
  getRelationExistenceQuery(query, parentQuery, columns = ['*']) {
    ...
    return query.select(columns).whereColumn(this.getQualifiedForeignKeyName(), '=', query.qualifyColumn(this._ownerKey));
  }
