...
export class FedacoBuilder extends mixinGuardsAttributes(
  mixinQueriesRelationShips(
    mixinBuildQueries(mixinForwardCallToQueryBuilder(class {}))
  )
) {
  constructor(_query) {
    super()
  ...
  orWhere(column, operator = null, value = null) {
    ;[value, operator] = this._query._prepareValueAndOperator(
      value,
      operator,
      arguments.length === 2
    )
  ...
  latest(column = null) {
    ...
    if (isBlank(column)) {
      column =
        (_a = this._model.getCreatedAtColumn()) !== null && _a !== void 0
          ? _a
          : 'created_at'
    }
  ...
  fromQuery(query, bindings = []) {
    return __awaiter(this, void 0, void 0, function* () {
      return this.hydrate(
        yield this._query.getConnection().select(query, bindings)
      )
    })
  }
      ...
      if (!_relation) {
        throw new Error(
          `RelationNotFoundException ${
            this.getModel().constructor.name
          } ${name}`
        )
      }
      return _relation
  ...
  paginate(page = 1, pageSize, columns = ['*']) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const results =
        total > 0 ? yield this.forPage(page, pageSize).get(columns) : []
      return {
        items: results,
        total,
        pageSize,
  ...
  addUpdatedAtColumn(values) {
    ...
    values = Object.assign(Object.assign({}, values), {
      [column]:
        column in values ? values[column] : this._model.freshTimestampString(),
    })
  ...
  _groupWhereSliceForScope(query, whereSlice) {
    ...
    if (whereBooleans.includes('or')) {
      query._wheres.push(
        this._createNestedWhere(whereSlice, nth(0, whereBooleans))
      )
    } else {
  ...
  with(relations, callback) {
    ...
    this._eagerLoad = Object.assign(
      Object.assign({}, this._eagerLoad),
      eagerLoad
    )
    return this
  }
