...
export const HasManyThroughColumn = makePropDecorator('Fedaco:HasManyThroughColumn', (p) => (Object.assign({
  isRelation: true, type: 'HasManyThrough', _getRelation: function(m, relation) {
    ...
    const r = new HasManyThrough(m._newRelatedInstance(clazz).newQuery(), m, through, firstKey, secondKey, p.localKey || m.getKeyName(), p.secondLocalKey || through.getKeyName());
    if (p.onQuery) {
      p.onQuery(r);
    }
    return r;
