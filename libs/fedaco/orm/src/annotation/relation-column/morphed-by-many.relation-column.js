...
export const MorphedByManyColumn = makePropDecorator('Fedaco:MorphedByMany', (p) => (Object.assign({
  isRelation: true, type: 'MorphedByMany', _getRelation: function(m, relation) {
    ...
    const r = new MorphToMany(instance.newQuery(), m, p.name, p.table, p.foreignPivotKey, p.relatedPivotKey, p.parentKey, p.relatedKey, relation, true);
    if (p.onQuery) {
      p.onQuery(r);
    }
    return r;
