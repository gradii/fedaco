...
export const MorphToManyColumn = makePropDecorator('Fedaco:MorphToManyColumn', (p) => (Object.assign({
  isRelation: true, type: 'MorphToMany', _getRelation: function(m, relation) {
    ...
    const r = new MorphToMany(instance.newQuery(), m, p.name, table, foreignPivotKey, relatedPivotKey, p.parentKey || m.getKeyName(), p.relatedKey || instance.getKeyName(), caller, p.inverse);
    if (p.onQuery) {
      p.onQuery(r);
    }
    return r;
  }
}, p)), FedacoRelationColumn, (target, name, decorator) => {
