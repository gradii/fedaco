...
export const HasOneThroughColumn = makePropDecorator('Fedaco:HasOneThrough', (p) => (Object.assign({
  isRelation: true, type: 'HasOneThrough', _getRelation: function(m, relation) {
    ...
    const r = new HasOneThrough(m._newRelatedInstance(clazz).newQuery(), m, through, firstKey, secondKey, p.localKey || m.getKeyName(), p.secondLocalKey || through.getKeyName());
    if (p.onQuery) {
      p.onQuery(r);
    }
    return r;
