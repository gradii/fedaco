...
export function mixinAggregate(base) {
  return class _Self extends base {
    ...
    _setAggregate(func, columns) {
      this._aggregate = new AggregateFragment(createIdentifier(func), columns.map(it => createColumnReferenceExpression(it)));
    ...
    aggregate(func, columns = ['*']) {
      return __awaiter(this, void 0, void 0, function* () {
        const results = yield this.cloneWithout(this._unions.length > 0 ?
          [] :
          ['columns'])
    ...
    exists(columns = '*') {
      return __awaiter(this, void 0, void 0, function* () {
        let results = yield this._connection.select(this._grammar.compileExists(this), this.getBindings(), !this._useWriteConnection);
      ...
      return __awaiter(this, void 0, void 0, function* () {
        if (this._groups.length > 0 || this._havings.length > 0) {
          ...
          if (clone._columns.length === 0 && this._joins.length > 0) {
            clone._columns = [
              new ColumnReferenceExpression(new PathExpression([
                this._from,
                createIdentifier('*')
              ]))
            ];
          ...
          return yield this.newQuery().from(rawSqlBindings('(' + clonedSql + ') as ' +
            this._grammar.quoteTableName('aggregate_table'), clonedBindings, 'from'))._setAggregate('count', this._withoutSelectAliases(columns)).get();
        }
