  ...
  return class _Self extends base {
    chunk(count, signal) {
      ...
      return new Observable((observer) => {
        ...
        subject.pipe(signal ? bufferWhen(() => {
          ...
          return signal;
        }) : map(it => [it]), mergeMap((items) => of(...items)), concatMap((page) => {
          return from(clone
            .forPage(page, count).get()).pipe(tap((results) => {
    ...
    each(count = 1000, signal) {
      return this.chunk(count, signal).pipe(mergeMap(({ results, page }) => {
        return of(...results.map((it, idx) => {
    ...
    chunkById(count, column, alias, signal) {
      ...
      column = column !== null && column !== void 0 ? column : this.defaultKeyName();
      ...
      return new Observable((observer) => {
        ...
        subject.pipe(signal ? bufferWhen(() => {
          ...
          return from(clone
            .forPageAfterId(count, lastId, column).get()).pipe(tap((results) => {
            if (!results || results.length == 0) {
              subject.complete();
              observer.complete();
            } else {
              ...
              if (isBlank(lastId)) {
                throw new Error(`RuntimeException The chunkById operation was aborted` +
    ...
    eachById(count = 1000, column, alias, signal) {
      return this.chunkById(count, column, alias, signal).pipe(mergeMap(({ results, page }) => {
        return of(...results.map((it, idx) => {
          return { item: it, index: (page - 1) * count + idx };
        }));
      }));
    ...
    when(condition, callback, defaultCallback) {
      var _a, _b;
      if (condition) {
        return (_a = callback(this, condition)) !== null && _a !== void 0 ? _a : this;
      }
      if (defaultCallback) {
        return (_b = defaultCallback(this, condition)) !== null && _b !== void 0 ? _b : this;
      }
      return this;
    }
