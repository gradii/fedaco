...
export function mixinWherePredicate(base) {
  return class _Self extends base {
    ...
    whereIn(column, values, conjunction = 'and', not = false) {
      const expression =
        SqlParser.createSqlParser(column).parseUnaryTableColumn()
      let subQuery,
        valueArray = []
      if (this.isQueryable(values)) {
        subQuery = this._createSubQuery('where', values)
      } else {
        valueArray = values.map((it) =>
          it instanceof RawExpression
            ? it
            : new BindingVariable(new RawExpression(it))
        )
      }
      this.addWhere(
        new InPredicateExpression(expression, valueArray, subQuery, not),
        conjunction
      )
