...
export class PostgresQueryGrammar extends QueryGrammar {
  ...
  _prepareUpdateAst(builder, values) {
    if (builder._joins.length > 0) {
      ...
      const ast = new UpdateSpecification(builder._from, columnNodes, new WhereClause(new ConditionExpression([
        new InPredicateExpression(new ColumnReferenceExpression(new PathExpression([new Identifier('ctid')])), [], new NestedExpression('where', inBuilder))
      ])));
      return ast;
    }
  ...
  _prepareDeleteAstWithJoins(builder) {
    if (builder._joins.length > 0) {
      ...
      const ast = new DeleteSpecification(builder._from, new WhereClause(new ConditionExpression([
        new InPredicateExpression(new ColumnReferenceExpression(new PathExpression([new Identifier('ctid')])), [], new NestedExpression('where', inBuilder))
      ])));
      return ast;
    }
