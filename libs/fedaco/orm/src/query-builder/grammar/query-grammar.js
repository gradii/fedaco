...
export class QueryGrammar extends BaseGrammar {
  ...
  _prepareUpdateAst(builder, values) {
    ...
    for (const [key, value] of Object.entries(values)) {
      ...
      columnNodes.push(new AssignmentSetClause(columnNode, bindingVariable(value, 'update')));
    }
    const ast = new UpdateSpecification(builder._from, columnNodes, builder._wheres.length > 0 ? new WhereClause(new ConditionExpression(builder._wheres)) : undefined);
  ...
  compileInsert(builder, values, insertOption = 'into') {
    ...
    const ast = new InsertSpecification(insertOption, new ValuesInsertSource(false, sources.map((columnValues) => {
  ...
  compileInsertUsing(builder, columns, nestedExpression) {
    const ast = new InsertSpecification('into', new ValuesInsertSource(false, [], nestedExpression), columns.map((it) => {
      return SqlParser.createSqlParser(it).parseColumnAlias();
    }), builder._from);
    const visitor = this._createVisitor(builder);
    return ast.accept(visitor);
  ...
  _prepareAggregateAst(builder, ast) {
    if (builder._unions.length > 0) {
      if (builder._aggregate) {
        ast = new QuerySpecification(new SelectClause([
          new SelectScalarExpression(new FunctionCallExpression(builder._aggregate.aggregateFunctionName, builder._aggregate.aggregateColumns), createIdentifier('aggregate'))
        ]), new FromClause(new FromTable(new TableReferenceExpression(new ParenthesizedExpression(ast), createIdentifier('temp_table')))));
      }
    }
    return ast;
  }
  ...
  _prepareSelectAst(builder) {
    ...
    let ast = new QuerySpecification(selectClause, builder._from ? new FromClause(builder._from, builder._joins) : undefined, whereClause);
    if (builder._limit >= 0) {
      ast.limitClause = new LimitClause(builder._limit);
    }
    if (builder._offset >= 0) {
  ...
  _prepareDeleteAstWithJoins(builder) {
    const ast = new DeleteSpecification(builder._from, builder._wheres.length > 0 ? new WhereClause(new ConditionExpression(builder._wheres)) : undefined);
    if (builder._joins.length > 0) {
      ast.fromClause = new FromClause(builder._from, builder._joins);
    }
    if (builder._limit >= 0) {
