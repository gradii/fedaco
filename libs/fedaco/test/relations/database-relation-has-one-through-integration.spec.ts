...
describe('test database fedaco has one through integration', () => {
  ...
  it('eager loading a relation with custom intermediate and local key', async () => {
    await seedData();
    const contract = (await HasOneThroughIntermediateTestPosition.createQuery().with('contract').first()).contract;
  ...
  it('where has on a relation with custom intermediate and local key', async () => {
    await seedData();
    const position = await HasOneThroughIntermediateTestPosition.createQuery()
      .whereHas('contract', (query) => {
  ...
  it('first or fail throws an exception', async () => {
    ...
    }).rejects.toThrow('ModelNotFoundException No query results for model [HasOneThroughTestContract].');
  });
  ...
  it('find or fail throws an exception', async () => {
    ...
    await expect(async () => {
      await (await HasOneThroughTestPosition.createQuery().first()).NewRelation('contract').findOrFail(1);
    }).rejects.toThrow('ModelNotFoundException');
  });

  it('first retrieves first record', async () => {
    await seedData();
    const contract = await (await HasOneThroughTestPosition.createQuery().first()).NewRelation('contract').first();
  ...
  it('only proper columns are selected if provided', async () => {
    await seedData();
    const contract = await (await HasOneThroughTestPosition.createQuery().first())
      .NewRelation('contract')
      .first(['title', 'body']);
  ...
  it('eager loading loads related models correctly', async () => {
    await seedData();
    const position = await HasOneThroughSoftDeletesTestPosition.createQuery().with('contract').first();
