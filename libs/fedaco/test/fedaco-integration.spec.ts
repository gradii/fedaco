...
describe('test database fedaco integration', () => {
  ...
  it('basic model retrieval', async () => {
    ...
    let model: FedacoTestUser = await factory.NewQuery().where('email', 'linbolen@gradii.com').first();
    expect(model.email).toBe('linbolen@gradii.com');
  ...
  it('pluck', async () => {
    ...
    const keyed = await FedacoTestUser.createQuery().oldest('id').pluck('users.email', 'users.id');
  ...
  it('pluck with column name containing a space', async () => {
    ...
    const keyed = await FedacoTestUserWithSpaceInColumnName.createQuery().oldest('id').pluck('email_address', 'id');
    expect(simple).toEqual(['linbolen@gradii.com', 'xsilen@gradii.com']);
    expect(keyed).toEqual({
      1: 'linbolen@gradii.com',
      2: 'xsilen@gradii.com',
  ...
  it('find or fail with single id throws model not found exception', async () => {
    ...
    }).rejects.toThrow('ModelNotFoundException No query results for model [FedacoTestUser] 1');
  });
  ...
  it('find or fail with multiple ids throws model not found exception', async () => {
    ...
    }).rejects.toThrow('ModelNotFoundException No query results for model [FedacoTestUser] [1,2]');
  });
  ...
  it('where has on nested self referencing belongs to relationship', async () => {
    ...
    // @ts-ignore
    const results: FedacoTestPost[] = await FedacoTestPost.createQuery()
      .whereHas('parentPost.parentPost', (query: FedacoBuilder) => {
  ...
  it('where has on self referencing has many relationship', async () => {
    ...
    // @ts-ignore
    const results: FedacoTestPost[] = await FedacoTestPost.createQuery()
      .whereHas('childPosts', (query: FedacoBuilder) => {
  ...
  it('where has on nested self referencing has many relationship', async () => {
    ...
    // @ts-ignore
    const results: FedacoTestPost[] = await FedacoTestPost.createQuery()
      .whereHas('childPosts.childPosts', (query: FedacoBuilder) => {
        query.where('name', 'Child Post');
      })
      .get();
  ...
  it('basic has many eager loading', async () => {
    ...
    user = await FedacoTestUser.createQuery().with('posts').where('email', 'linbolen@gradii.com').first();
    expect(head(await user.posts).name).toBe('First Post');
    const post = await FedacoTestPost.createQuery().with('user').where('name', 'First Post').get();
  ...
  it('eager loaded morph to relations on another database connection', async () => {
    ...
    const secondConnectionPost = (await FedacoTestPhoto.useConnection('second_connection').with('imageable').first())
      .imageable;
  ...
  it('belongs to many custom pivot', async () => {
    ...
    const johnWithFriends = await FedacoTestUserWithCustomFriendPivot.createQuery().with('friends').find(1);
  ...
  it('updating child model touches parent', async () => {
    ...
    expect(isSameDay(old, (await FedacoTouchingUser.createQuery().first()).updated_at)).toBeTruthy();
    expect(isSameDay(old, (await FedacoTouchingPost.createQuery().first()).updated_at)).toBeTruthy();
