...
import { isAnyEmpty, isArray, isBlank, isNumber, isString, snakeCase } from '@gradii/nanofn';
...
export interface QueriesRelationships {
  ...
  whereHas(relation: string | Relation, callback?: (q: FedacoBuilder) => void, operator?: string, count?: number): this;
  ...
  hasMorph(
    ...
    count?: number,
    conjunction?: string,
    callback?: ((q: FedacoBuilder) => void) | null,
  ): this;
...
`);
      ...
      const hasQuery: FedacoBuilder = (relation as any)[method](
        (relation as Relation).getRelated().NewQueryWithoutRelationships(),
        this,
      );
        ...
        (query: FedacoBuilder) => {
          for (const type of morphedTypes) {
            query.orWhere((q: FedacoBuilder) => {
              const belongsTo: BelongsTo = this._getBelongsToRelation(
                relation as MorphTo,
                type,
              ) as unknown as BelongsTo;
              if (callback) {
      ...
      for ([name, constraints] of Object.entries(this._parseWithRelations(relations))) {
        ...
        if (func) {
          ...
          const wrappedColumn = this.getQuery()
            .getGrammar()
            .wrap(column === '*' ? column : relation.getRelated().QualifyColumn(hashedColumn));
        ...
        const query = relation.getRelationExistenceQuery(
          ...
          this as unknown as FedacoBuilder,
          raw(expression),
        ); // .setBindings([], 'select');
        ...
        alias = alias ?? snakeCase(`${name} ${func} ${column}`.replace(/^[0-9A-Za-z]\s+_/u, ''));
        if (func === 'exists') {
          const sql = query.toSql();
          this.selectRaw(`exists(${sql.result}) as ${this.getQuery()._grammar.wrap(alias)}`, sql.bindings).withCasts({
            [alias]: 'boolean',
          });
    ...
    public withCount(this: FedacoBuilder & _Self, relations: RelationParam | RelationParams): FedacoBuilder {
      return this.withAggregate(isArray(relations) ? relations : [...arguments], '*', 'count');
    }

    /* Add subselect queries to include the max of the relation's column. */
    public withMax(this: FedacoBuilder & _Self, relation: RelationParams, column: string): FedacoBuilder {
      return this.withAggregate(relation, column, 'max');
    }

    /* Add subselect queries to include the min of the relation's column. */
    public withMin(this: FedacoBuilder & _Self, relation: RelationParams, column: string): FedacoBuilder {
      return this.withAggregate(relation, column, 'min');
    }

    /* Add subselect queries to include the sum of the relation's column. */
    public withSum(this: FedacoBuilder & _Self, relation: RelationParams, column: string): FedacoBuilder {
      return this.withAggregate(relation, column, 'sum');
    }

    /* Add subselect queries to include the average of the relation's column. */
    public withAvg(this: FedacoBuilder & _Self, relation: RelationParams, column: string): FedacoBuilder {
