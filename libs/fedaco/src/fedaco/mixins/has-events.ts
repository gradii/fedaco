...
export function mixinHasEvents<T extends Constructor<any>>(base: T) {
  ...
  return class _Self extends base {
    ...
    public AddObservableEvents(observables: any[] | any) {
      this._observables = uniq([...this._observables, ...(isArray(observables) ? observables : arguments)]);
    }
    ...
    _fireModelEvent(this: Model & _Self, event: string, halt = true) {
      ...
      return !isAnyEmpty(result)
        ? result
        : /* static */ (<any>this.constructor).dispatcher[method](`fedaco.${event}: ${this.GetTable()}`, this);
    }
