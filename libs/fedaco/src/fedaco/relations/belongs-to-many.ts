...
import type { InteractsWithPivotTable } from './concerns/interacts-with-pivot-table';
...
export class BelongsToMany extends mixinInteractsWithDictionary(mixinInteractsWithPivotTable(Relation)) {
  ...
  _performJoin(query: FedacoBuilder | null = null) {
    ...
    query.join(this._table, this.getQualifiedRelatedKeyName(), '=', this.getQualifiedRelatedPivotKeyName());
    return this;
  }
  ...
  public addEagerConstraints(models: Model[]) {
    ...
    this._query[whereIn](this.getQualifiedForeignPivotKeyName(), this.getKeys(models, this._parentKey));
  }
  ...
  public wherePivot(column: string, value: any): FedacoBuilder;
  // public wherePivot(column: string,
  //                   value: any,
  //                   conjunction?): QueryBuilder;
  public wherePivot(column: string, operator?: string, value?: any, conjunction?: 'and' | 'or' | string): FedacoBuilder;
  public wherePivot(column: string, operator?: any, value?: any, conjunction: 'and' | 'or' = 'and'): FedacoBuilder {
  ...
  public wherePivotBetween(column: string, values: any[], conjunction = 'and', not = false) {
  ...
  public wherePivotIn(column: string, values: any, conjunction = 'and', not = false) {
  ...
  public async firstOrCreate(attributes: any = {}, values: any = {}, joining: any[] = [], touch = true) {
    let instance = await this._related.NewQuery().where(attributes).first();
    if (isBlank(instance)) {
  ...
  public async updateOrCreate(attributes: any[], values: any = {}, joining: any[] = [], touch = true) {
  ...
  public async find(id: any, columns: any[] = ['*']): Promise<Model | Model[]> {
    ...
    return this.where(this.getRelated().GetQualifiedKeyName(), '=', this._parseIds(id)).first(columns);
  }
  ...
  public async paginate(page = 1, pageSize?: number, columns: any[] = ['*']) {
  ...
  public chunk(count: number, signal?: Observable<any>): Observable<{ results: any[]; page: number }> {
    this._prepareQueryBuilder();
  ...
  ): Observable<{ results: any; page: number }> {
    this._prepareQueryBuilder();
  ...
  public getRelationExistenceQueryForSelfJoin(
    query: FedacoBuilder,
