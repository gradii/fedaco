...
export class MorphOne extends mixinCanBeOneOfMany(
  ...
  public addOneOfManyJoinSubQueryConstraints(join: JoinClauseBuilder) {
    join
      .on(this._qualifySubSelectColumn(this._morphType), '=', this.qualifyRelatedColumn(this._morphType))
      .on(this._qualifySubSelectColumn(this._foreignKey), '=', this.qualifyRelatedColumn(this._foreignKey));
  }
