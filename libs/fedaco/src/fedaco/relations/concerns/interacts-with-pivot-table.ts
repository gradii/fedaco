  ...
  return class _Self extends base {
    ...
    public async toggle(this: BelongsToMany & _Self, ids: any, touch = true): Promise<PivotTableData> {
      ...
      const detach = Object.values(
        intersection((await this.newPivotQuery().pluck(this.relatedPivotKey)) as any[], Object.keys(records)),
      );
      if (detach.length > 0) {
      ...
      if (touch && (changes['attached'].length || changes['detached'].length)) {
    ...
    /* Sync the intermediate tables with a list of IDs or collection of models. */
    public async sync(
      this: BelongsToMany & _Self,
      ids: Collection | Model | any[] | any,
      detaching = true,
    ...
    public async updateExistingPivot(
      ...
      if (this._using && !this._pivotWheres.length && !this._pivotWhereIns.length && !this._pivotWhereNulls.length) {
      ...
      const updated = await this.newPivotStatementForId(this._parseId(id)).update(this._castAttributes(attributes));
      if (touch) {
        await this.touchIfTouching();
      }
      return updated;
    ...
    /* Update an existing pivot record on the table via a custom class. */
    async _updateExistingPivotUsingCustomClass(
      this: BelongsToMany & _Self,
      id: any,
      attributes: any,
      touch: boolean,
    ...
    /* Attach a model to the parent. */
    public async attach(this: BelongsToMany & _Self, id: any, attributes: any = {}, touch = true): Promise<void> {
      if (this._using) {
        await this._attachUsingCustomClass(id, attributes);
      } else {
        await this.newPivotStatement().insert(this._formatAttachRecords(this._parseIds(id), attributes));
      }
      if (touch) {
        await this.touchIfTouching();
      }
    ...
    /* Attach a model to the parent using a custom class. */
    async _attachUsingCustomClass(this: BelongsToMany & _Self, id: any, attributes: any[]): Promise<void> {
    ...
    /* Create a new pivot attachment record. */
    _baseAttachRecord(this: BelongsToMany & _Self, id: number, timed: boolean): Record<string, any> {
    ...
    async _getCurrentlyAttachedPivots(this: BelongsToMany & _Self): Promise<any[]> {
      return (await this.newPivotQuery().get()).map((record: any) => {
        ...
        const pivot = clazz.fromRawAttributes(this.getParent(), /* cast type array */ record, this.getTable(), true);
        return pivot.SetPivotKeys(this._foreignPivotKey, this._relatedPivotKey);
      });
    }

    /* Create a new pivot model instance. */
    public newPivot(this: BelongsToMany & _Self, attributes: Record<string, any> = {}, exists = false) {
    ...
    /* Set the columns on the pivot table to retrieve. */
    public withPivot(this: /* BelongsToMany */ any & this, columns: any[] | any, ...cols: any[]): this {
      this._pivotColumns = [...this._pivotColumns, ...(isArray(columns) ? columns : arguments)];
      return this;
    }
