...
export interface BuildQueries {
  ...
  chunkById(count: number, column?: string, alias?: string): Observable<{ results: any; page: number }>;

  eachById(count: number, column?: string, alias?: string): Observable<{ item: any; index: number }>;
...
export function mixinBuildQueries<T extends Constructor<any>>(base: T): BuildQueriesCtor & T {
  return class _Self extends base {
    ...
    public each(count = 1000, concurrent?: number): Observable<{ item: any; index: number }> {
     ...
     */
    public chunkById<T extends Model = any>(
      this: QueryBuilder & FedacoBuilder & _Self,
      count: number,
      column?: string,
      alias?: string,
    ): Observable<{ results: T[]; page: number }> {
      if (!(count > 0)) {
        return EMPTY;
      }
    ...
    public when(condition: boolean, callback: (q: this, condition: boolean) => any, defaultCallback?: Function): this {
      if (condition) {
