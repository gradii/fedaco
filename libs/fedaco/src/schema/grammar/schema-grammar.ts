...
export class SchemaGrammar extends BaseGrammar {
  ...
  public compileRenameColumn(
    ...
    return `alter table ${this.wrapTable(blueprint)} rename column ${this.wrap(
      command.from,
  ...
  protected getDefaultValue(value: any) {
    ...
    return isBoolean(value) ? `'${/* cast type int */ value ? 1 : 0}'` : `'${/* cast type string */ value}'`;
  }
  ...
  public compileEnableForeignKeyConstraints(): string {
    throw new Error('LogicException This database driver does not support enable foreign key constraints.');
  }

  public compileDisableForeignKeyConstraints(): string {
    throw new Error('LogicException This database driver does not support disable foreign key constraints.');
  }
