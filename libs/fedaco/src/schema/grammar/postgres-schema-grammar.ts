...
export class PostgresSchemaGrammar extends SchemaGrammar {
  ...
  public compileColumns(schema: string, table: string) {
    return `select a.attname                            as name,
                   ...
                   (select tc.collcollate
                    ...
                    where tc.oid = a.attcollation) as collation, not a.attnotnull as nullable, (select pg_get_expr(adbin, adrelid) from pg_attrdef where c.oid = pg_attrdef.adrelid and pg_attrdef.adnum = a.attnum) as default, ${'a.attgenerated as generated, '}col_description(c.oid, a.attnum) as comment
  ...
  public compileIndexes(schema: string, table: string) {
    return `select ic.relname                                     as name,
            ...
            where tc.relname = ${this.quoteString(table)}
              and tn.nspname = ${this.quoteString(schema)}
  ...
  public compileForeignKeys(schema: string, table: string) {
    return `select c.conname                                       as name,
            ...
            where c.contype = 'f'
              and tc.relname = ${this.quoteString(table)}
              and tn.nspname = ${this.quoteString(schema)}
  ...
  public compileTableComment(blueprint: Blueprint, command: ColumnDefinition) {
    return `comment on table ${this.wrapTable(blueprint)} is ${`'${command.comment.replace(/'+/g, "''")}'`}`;
  }
  ...
  protected typeTime(column: ColumnDefinition) {
    return `time${isBlank(column.precision) ? '' : `(${column.precision})`} without time zone`;
  }

  /* Create the column definition for a time (with time zone) type. */
  protected typeTimeTz(column: ColumnDefinition) {
    return `time${isBlank(column.precision) ? '' : `(${column.precision})`} with time zone`;
  }
  ...
  protected typeTimestampTz(column: ColumnDefinition) {
    const columnType = `timestamp${isBlank(column.precision) ? '' : `(${column.precision})`} with time zone`;
  ...
  protected modifyIncrement(blueprint: Blueprint, column: ColumnDefinition) {
    if ((this.serials.includes(column.type) || column.generatedAs !== null) && column.autoIncrement) {
      return ' primary key';
    }
    return '';
  }
